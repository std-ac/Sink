...
export default eventHandler(async (event) => {
  ...
  if (slug && !reserveSlug.includes(slug) && slugRegex.test(slug) && cloudflare) {
    ...
    if (link) {
      ...
      const target = redirectWithQuery ? withQuery(link.url + tail, getQuery(event)) : link.url + tail
